...
@description('Named backend pools that group existing backends for priority/weight load balancing. Each item: { name, services: [{ backend, priority, weight }] }')
...
resource inferenceBackend 'Microsoft.ApiManagement/service/backends@2024-06-01-preview' =  [for (config, i) in aiServicesConfig: if(length(aiServicesConfig) > 0) {
  ...
  properties: {
    ...
    protocol: 'http'
    circuitBreaker: ((config.?circuitBreaker ?? false) || configureCircuitBreaker) ? {
      rules: [
      {
        failureCondition: {
        count: 1
...
// Named, purpose-built pools (e.g. a priority-based PTU/TPM spillover pool for a single model)
resource namedBackendPools 'Microsoft.ApiManagement/service/backends@2024-06-01-preview' = [for pool in backendPoolsConfig: {
  name: pool.name
  parent: apimService
  #disable-next-line BCP035
  properties: {
    description: pool.?description ?? 'Named load-balanced pool'
    type: 'Pool'
    pool: {
      services: [for svc in pool.services: {
        id: '/backends/${svc.backend}'

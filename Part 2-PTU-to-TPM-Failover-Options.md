...
An exploration of **every** way a PTU (Provisioned Throughput Unit) primary can spill over to a TPM (Standard / PayGo, token-per-minute) secondary, and which ones are worth demonstrating in this lab.

> Status: working document. Scenarios 1 and 2 are implemented in [policy.xml](policy.xml) / [main.bicep](main.bicep); the rest are candidates. Behavioral claims about APIM and Foundry were checked against Microsoft Learn on 2026-09-17 and are linked inline.
...
### Scenario 1 — Policy retry, two deployments on ONE Foundry resource ✅ implemented

`gpt-5-mini-ptu` → `gpt-5-mini-tpm` on the same `foundry2` endpoint.

```mermaid
flowchart LR
    c([Client]) --> apim[APIM]
    apim -->|attempt 1| ptu["gpt-5-mini-ptu<br/>capacity 1"]
    ptu -.->|429| apim
    apim -->|attempt 2 · rewrite deployment| tpm["gpt-5-mini-tpm<br/>capacity 20"]
```

- **Decision point:** `<retry>` inside `<backend>`, rewriting the URL path *and* the body `model` field.
- **Granularity:** the *same* request fails over — zero user-visible failures.
- **Why a pool can't do this:** both deployments share one endpoint host, so APIM backends (host-scoped) cannot distinguish them. See [Part 1a1-DeploymentIsResourceScoped.md](Part%201a1-DeploymentIsResourceScoped.md).
- **Pros:** cheapest topology (one resource, one quota footprint), no extra region, deterministic.
- **Cons:** custom policy to maintain; body buffering required (`buffer-request-body="true"`); does not help if the whole resource/region is down.

### Scenario 2 — Native backend pool + circuit breaker, two resources ✅ implemented
...
### Scenario 2b — Pool + retry, to also recover the triggering request

Same pool as Scenario 2, plus a `<retry>` on 429. Microsoft's documented ["switch backend when error received"](https://learn.microsoft.com/azure/api-management/retry-policy#examples) pattern names the target explicitly on each attempt:
...
**Recommended production shape.** Deliberately *not* implemented in this lab — [README.md](README.md) explains that omitting it keeps the 429 visible and preserves the contrast with Scenario 1.

### Scenario 3 — Platform-native provisioned **spillover** (no gateway involved)
...
### Scenario 4 — Weighted pool (percentage bleed, not failover)

Pool members with `weight` 90/10 instead of priorities: 90 % to PTU, 10 % pre-emptively to TPM.

- **Use when:** you want to smooth bursts *before* hitting 429, or to keep the TPM path warm.
- **Cons:** deliberately leaks paid-for PTU capacity; not a true failover — combine with priority groups (weights inside a priority tier).

### Scenario 5 — Token-budget-driven routing (proactive, not reactive)
...
### Scenario 6 — Utilization-metric-driven routing
...
### Scenario 7 — Client-side / SDK failover

The app holds two clients (PTU endpoint, TPM endpoint) and catches 429 itself.

- **Pros:** no gateway dependency; full control of streaming retry semantics.
- **Cons:** logic duplicated in every app and language; no central governance, metering, or policy; credentials for both endpoints spread everywhere. Largely superseded once APIM exists, but it is the starting point most teams arrive from.

### Scenario 8 — Cross-region spillover

PTU in region A, TPM in region B (or a Global Standard deployment as the overflow).
...
### Scenario 9 — Multi-layer (gateway + traffic manager)

Two APIM instances (or two regions of a Premium multi-region APIM) behind Front Door, each fronting a regional PTU/TPM pair.

- **Pros:** handles gateway failure, not just model throttling.
- **Cons:** cost and complexity; the failover reason is opaque at the edge. Out of scope for this lab.

### Scenario 10 — Cache first, fail over second

`llm-semantic-cache-lookup` / `-store` in front of everything. Cache hits never consume PTU or TPM.

- Not a failover mechanism, but it reduces how often failover triggers at all.

### Scenario 11 — Model-substitution fallback (degrade, don't fail)
...
### Scenario 12 — Queue / shed instead of spilling
...
|---|---|---|---|---|---|---|---|
| 1 | Policy retry, 2 deployments / 1 resource | APIM policy | ✅ | ❌ | ❌ | none | medium |
| 2 | Priority pool + circuit breaker | APIM pool | ❌ | ✅ | ✅ | 2nd resource | none |
| 2b | Pool + breaker + explicit retry | APIM pool+policy | ✅ | ✅ | ✅ | 2nd resource | low |
| 3 | Native provisioned spillover | Foundry | ✅ | ❌ (same resource) | ❌ | none | none |
| 4 | Weighted pool | APIM pool | n/a | ✅ | ✅ | 2nd resource | none |
| 5 | Token-budget routing | APIM policy | ✅ (pre-empts) | ✅ | ✅ | none | medium |
| 6 | Utilization-metric routing | External + APIM | ✅ (pre-empts) | ✅ | ✅ | automation | high |
| 7 | Client SDK failover | App | ✅ | ✅ | ✅ | none | n/a (app code) |
| 8 | Cross-region spillover | pool or policy | depends | ✅ | ✅ | 2nd region | low |
| 9 | Front Door + multi-region APIM | Edge | ❌ | ✅ | ✅ | high | n/a |
| 10 | Semantic cache | APIM policy | n/a | n/a | n/a | Redis | low |
| 11 | Model substitution | APIM policy | ✅ | ✅ | ✅ | none | low |
| 12 | Shed / queue | APIM policy | ❌ | n/a | n/a | none | low |
...
A 429 arrives before any response body, so pre-first-byte failover (Scenarios 1, 2b, 3) is safe. Once the first SSE chunk has been written to the client, no retry is possible and a mid-stream backend failure is unrecoverable.
...
- For native spillover (Scenario 3): the `x-ms-spillover-from-deployment` / `x-ms-deployment-name` / `x-ms-spillover-error` response headers, and the `IsSpillover` split on `Azure OpenAI Requests`.
...
1. **`x-served-backend` outbound header** — prerequisite for the rest. Without it, Scenario 2's spillover is invisible, because both pool members report the same `x-ms-region`.
2. **Scenario 11** — model substitution, reusing the existing `gpt-5-nano` deployment. Nearly free to add.
3. **Scenario 5** — token-limit pre-emptive routing. Shows proactive vs. reactive.
4. **Scenario 10** — semantic cache, if a Redis dependency is acceptable.

Not planned: **Scenario 2b** is documented in [README.md](README.md) but intentionally left out of the policy so the 429 stays observable. **Scenario 3** cannot be demonstrated without real provisioned capacity.

---

## 7. Open questions

- Should the Scenario 2 breaker trip on `5xx` as well as `429`, and what does that do to the demo's determinism?
...
| Eligible for **native** spillover (Scenario 3) | Not listed as supported | Yes | Yes |
...
- Every gateway-side failover pattern in the catalogue above — the SKU never changes the mechanism. (Native spillover, Scenario 3, is the exception: it has SKU prerequisites.)

### What the scope *does* change

1. **Your exposure to a regional capacity crunch.** Global draws on a much larger pool, so it is less likely to be blocked at *deployment* time or affected by one region's constraints; regional is the most exposed (Scenario 8's main argument). Neither changes how your own PTU ceiling saturates at runtime.
2. **What your spillover target may legally be.** A regional PTU usually exists *because* someone required in-region processing — Scenario 8 is off the table for that workload unless the secondary is in the same region.
3. **Whether Scenario 3 is even available.** Native spillover requires a global or data zone provisioned deployment; a regional PTU has to fail over at the gateway.
...
1. **Idle PTU is sunk cost, not saved cost.** With a reservation, every minute the gateway routes around the primary is money already spent producing nothing. This is the strongest argument for short circuit-breaker TTLs (Scenario 2) and for retry-based failover that returns to the primary immediately (Scenarios 1, 2b) over stateful breakers.
...
4. **Pre-emptive patterns get more attractive.** Scenarios 5 and 6 route away from the PTU *before* it throttles — tune the threshold to ~95–100 %, not 70 %, or you are voluntarily paying twice for the same tokens.
